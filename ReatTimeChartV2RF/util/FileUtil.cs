...
namespace RealtimeChart
{
    public static class FileUtil
    {
        ...
        /// <summary>
        /// 存储RFID 数据值，Phase，timestamp 顺序存储到文件中
        /// </summary>
        /// <param name="phase">rfid香味信息</param>
        /// <param name="timestamps">时间戳信息</param>
        /// <param name="description"></param>
        public static void writeTwoVector(List<double> phase, List<long> timestamps, string description)
        {
            description = ItemString.baseFolder + description + ".txt";
            StreamWriter sw = new StreamWriter(description);
            for (int i = 0; i < phase.Count && i < timestamps.Count; ++i)
            {
                sw.WriteLine(string.Format("{0} {1}", phase[i], timestamps[i]));
            }
            sw.Close();
            System.Diagnostics.Debug.WriteLine("writeTwoVector: OK saveto" + description);
        }
        /// <summary>
        /// 存储三个序列，其中description 为对应的文件名称
        /// </summary>
        ...
        public static void writeThreeVector(List<double> data1, List<double> data2, List<long> timestamp, string description)
        {
            ...
            sw.Close();
            System.Diagnostics.Debug.WriteLine("writeThreeVector: OK saveto"+ description);
        }
        /// <summary>
        /// 从文件中读取RFID data
        /// </summary>
        ...
        public static RFIDData readRFIDData(string filename)
        {
            ...
            while ((line = sr.ReadLine()) != null)
            {
                ...
                rFIDData.addPhaseTime(Convert.ToDouble(temp[0]), Convert.ToInt64(temp[1]));
            }
